...
class Sns_Megamenu_Model_Observer{
	...
	public function updateItemsLRWhileEdit($observer){
		...
		elseif($orderItemId ==0){
			...
			$order = Sns_Megamenu_Model_System_Config_Source_Position::FIRST;
	...
	public function setItemLR($observer){
		...
		try {
			if($model->getData('order_item')){
				...
				$data = $modelMenuitems->getData();
				if($model->getData('position_item') == Sns_Megamenu_Model_System_Config_Source_Position::AFTER){
					$lft = intval($data['rgt'])+1;
					$rgt = intval($data['rgt'])+2;
					$depth =  intval($data['depth']);
				}
				elseif($model->getData('position_item') == Sns_Megamenu_Model_System_Config_Source_Position::BEFORE){
					$lft = intval($data['lft']);
